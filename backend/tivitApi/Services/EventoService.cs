    ...
    public class EventoService : IEventoService
    {
        ...
        public async Task<object> criarEvento(EventoDTO eventoDTO)
        {
            try
            {
                // ===== Validações =====
                if (eventoDTO == null)
                    throw new ValidationException("Dados do evento não enviados.");

                if (string.IsNullOrWhiteSpace(eventoDTO.Titulo))
                    throw new ValidationException("O título do evento é obrigatório.");

                if (eventoDTO.Horario == default)
                    throw new ValidationException("O horário do evento é inválido.");
        ...
        public async Task<EventoDTO> obterProximoEvento()
        {
            ...
            try
            {
                ...
                if (proximoEvento == null)
                    throw new NotFoundException("Evento futuro não encontrado.");
